...
mediawiki_api_endpoint = 'https://data.lexbib.org/w/api.php'

# SPARQL endpoint
wikibase_sparql_endpoint = 'https://data.lexbib.org/query/sparql'

# Name of the Wikibase instance
wikibase_name = 'LexBib'

# URL of the main page of the Wikibase instance
wikibase_main_page = 'https://data.lexbib.org/wiki/Main_Page'
...
user_agent = 'OpenRefine-LexBib reconciliation interface'

# Regexes and group ids to extracts Qids and Pids from URLs
import re
q_re = re.compile(r'(<?https?://data.lexbib.org/(entity|wiki)/)?(Q[0-9]+)>?')
q_re_group_id = 3
p_re = re.compile(r'(<?https?://data.lexbib.org/(entity/|wiki/Property:))?(P[0-9]+)>?')
p_re_group_id = 3
...
identifier_space = 'http://data.lexbib.org/entity/'
schema_space = 'http://data.lexbib.org/prop/direct/'
...
qid_url_pattern = 'https://data.lexbib.org/wiki/{{id}}'
...
service_name = 'LexBib Reconciliation'
...
headers = {
    'User-Agent':service_name + ' (OpenRefine-LexBib reconciliation service)',
}
